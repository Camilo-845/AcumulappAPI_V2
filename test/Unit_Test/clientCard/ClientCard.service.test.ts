...
describe("ClientCardService", () => {
  ...
  describe("createClientCard", () => {
    it("should create a new client card with a unique code", async () => {
      ...
      (uniqueCodeUtils.generateAlphanumericCode as jest.Mock).mockReturnValue(
        uniqueCode,
      );
      (
        ClientCardRepository.prototype.existsByUniqueCode as jest.Mock
      ).mockResolvedValue(false);
      (ClientCardRepository.prototype.create as jest.Mock).mockResolvedValue(
        createdClientCard,
      );
      ...
      expect(
        ClientCardRepository.prototype.existsByUniqueCode,
      ).toHaveBeenCalledWith(uniqueCode);
    ...
    it("should throw an error if a unique code cannot be generated", async () => {
      ...
      await expect(
        clientCardService.createClientCard({ idClient: 1, idCard: 1 }),
      ).rejects.toThrow(
        new ApiError(
  ...
  describe("activateClientCard", () => {
    it("should throw an error if client card is not found", async () => {
      (
        ClientCardRepository.prototype.findByUniqueCode as jest.Mock
      ).mockResolvedValue(null);

      await expect(
        clientCardService.activateClientCard("nonexistent-code"),
      ).rejects.toThrow(
    ...
    it("should activate a client card", async () => {
      ...
      const updatedClientCard = { ...clientCard, idCardState: 4 };

      (
        ClientCardRepository.prototype.findByUniqueCode as jest.Mock
      ).mockResolvedValue(clientCard);
      (CardRepository.prototype.findById as jest.Mock).mockResolvedValue(card);
      (ClientCardRepository.prototype.update as jest.Mock).mockResolvedValue(
        updatedClientCard,
      );
      ...
      expect(ClientCardRepository.prototype.update).toHaveBeenCalledWith(
        clientCard.id,
        expect.any(Object),
      );
    });
  ...
  describe("addStampsToClientCard", () => {
    it("should add stamps to a client card", async () => {
      ...
      const result = await clientCardService.addStampsToClientCard(
        "test-code",
        1,
      );
    ...
    it("should throw an error if stamps exceed max stamps", async () => {
      ...
      await expect(
        clientCardService.addStampsToClientCard("test-code", 2),
      ).rejects.toThrow(
        new ApiError(
